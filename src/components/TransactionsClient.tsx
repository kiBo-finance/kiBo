...
export function TransactionsClient() {
  ...
  const [editingScheduledTransaction, setEditingScheduledTransaction] =
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">収入、支出、振替の記録と予定取引の管理</p>
        </div>
        <div className="flex gap-2">
          ...
          <Button onClick={() => setShowTransactionForm(true)} className="cursor-pointer">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div
              className={`text-2xl font-bold ${netIncome.isPositive() ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
            >
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{overdueTransactions.length}</div>
            <p className="text-xs text-muted-foreground">未処理の予定</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <TabsContent value="transactions" className="space-y-4">
          <TransactionFilters />
          <TransactionList onEdit={handleEditTransaction} showActions={true} />
        </TabsContent>
        ...
        <TabsContent value="scheduled" className="space-y-4">
          {overdueTransactions.length > 0 && (
            <Card className="border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-900/20">
              <CardHeader>
                <CardTitle className="text-red-600 dark:text-red-400">期限切れの予定取引</CardTitle>
                <CardDescription>
          ...
          <ScheduledTransactionList onEdit={handleEditScheduledTransaction} showActions={true} />
        </TabsContent>
      ...
      <Dialog open={showTransactionForm} onOpenChange={setShowTransactionForm}>
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingTransaction ? '取引を編集' : '新しい取引'}</DialogTitle>
          </DialogHeader>
          <TransactionForm
      ...
      <Dialog open={showScheduledForm} onOpenChange={setShowScheduledForm}>
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
          ...
          <div className="p-4 text-center text-muted-foreground">予定取引フォームは実装中です</div>
        </DialogContent>
      </Dialog>
    </div>
  )
