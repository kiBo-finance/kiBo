  ...
  const loadSettings = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const webhookSettings = settingsArray.find(
          (s: NotificationSettings) => s.type === 'WEBHOOK'
        )
        if (webhookSettings) {
  ...
  const testNotification = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage({
          type: 'success',
          text: 'テスト通知を送信しました（実際のリマインダーがある場合）',
        })
      } else {
      ...
      return (
        <div className="space-y-2">
          ...
          <Button
            ...
            size="sm"
            onClick={() =>
              window.open('https://support.discord.com/hc/ja/articles/228383668', '_blank')
            }
            className="cursor-pointer"
          >
            <ExternalLink className="mr-2 h-4 w-4" />
    ...
    <div className="space-y-6">
      ...
      {message && (
        ...
        >
          ...
          <AlertTitle>{message.type === 'error' ? 'エラー' : '成功'}</AlertTitle>
        ...
        <TabsContent value="settings" className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>現在アクティブな通知設定</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
                {settings
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setCurrentSettings({ ...currentSettings, type: value as 'DISABLED' | 'WEBHOOK' | 'EMAIL' })
                  }
                >
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setCurrentSettings({ ...currentSettings, webhookType: value as 'SLACK' | 'DISCORD' })
                      }
                    >
                      <SelectTrigger className="cursor-pointer">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="space-y-0.5">
                      ...
                      <p className="text-sm text-muted-foreground">支払い期日が近づいた時の通知</p>
                    </div>
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        setCurrentSettings({
                          ...currentSettings,
                          scheduledTransactionReminders: checked,
                        })
                      }
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="space-y-0.5">
                      <Label>期限切れ通知</Label>
                      <p className="text-sm text-muted-foreground">期限を過ぎた予定取引の通知</p>
                    </div>
                    <Switch
                  ...
                  <div className="flex items-center justify-between">
                    <div className="space-y-0.5">
                      <Label>予算アラート</Label>
                      <p className="text-sm text-muted-foreground">予算超過時の通知（未実装）</p>
                    </div>
                    <Switch
              ...
              <div className="flex gap-2 pt-4">
                <Button
                  ...
                  disabled={
                    saving ||
                    (currentSettings.type === 'WEBHOOK' &&
                      (!currentSettings.webhookUrl || !currentSettings.webhookType))
                  }
                  className="cursor-pointer"
                >
                  {saving ? '保存中...' : '設定を保存'}
        ...
        <TabsContent value="logs">
          <Card>
            <CardHeader>
              ...
              <CardDescription>送信された通知の履歴（未実装）</CardDescription>
            </CardHeader>
            <CardContent>
