...
export function ScheduledTransactionsOverview() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const upcomingTxs = transactions
          .filter((tx: ScheduledTransaction) => {
            const txDate = new Date(tx.dueDate)
            const txDateOnly = new Date(txDate.getFullYear(), txDate.getMonth(), txDate.getDate())
  ...
  const totalPending =
    todayTransactions.length + overdueTransactions.length + upcomingTransactions.length
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>今日の予定と期限切れの取引をチェック</CardDescription>
          </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  ...
                  {overdueTransactions.length > 2 && (
                    <Link to={"/dashboard/scheduled?status=overdue" as any}>
              ...
              <div>
                ...
                <div className="space-y-2">
                  ...
                  {todayTransactions.length > 2 && (
                    <Link to={"/dashboard/scheduled?status=today" as any}>
                      <Button variant="ghost" size="sm" className="w-full">
