...
export function ScheduledCalendarClient() {
  ...
  const getTransactionsForDate = (date: Date) => {
    return transactions.filter((tx) => isSameDay(new Date(tx.dueDate), date))
  }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">月別の予定取引をカレンダー形式で確認できます</p>
          </div>
        </div>
                  ...
                  <div className="grid grid-cols-7 gap-1">
                    {calendarDays.map((day) => {
                      ...
                      return (
                        <div
                          key={day.toISOString()}
                          className={`min-h-[120px] cursor-pointer rounded-lg border p-2 transition-colors ${
                            isCurrentMonth
                              ? 'bg-white hover:bg-gray-50'
                              : 'bg-gray-50 text-gray-400'
                          } ${isTodayDate ? 'border-blue-500 bg-blue-50' : 'border-gray-200'} ${isSelected ? 'ring-2 ring-blue-500' : ''} `}
                          onClick={() => setSelectedDate(day)}
                        >
                          <div
                            className={`mb-1 text-sm font-medium ${isTodayDate ? 'text-blue-600' : ''} ${!isCurrentMonth ? 'text-gray-400' : ''} `}
                          >
                            {format(day, 'd')}
                          </div>
                          ...
                          <div className="space-y-1">
                            {dayTransactions.slice(0, 3).map((transaction) => (
                              <Link
                                key={transaction.id}
                                to={`/dashboard/scheduled/${transaction.id}` as any}
                ...
                selectedDateTransactions.length > 0 ? (
                  <div className="space-y-3">
                    {selectedDateTransactions.map((transaction) => (
                      <Link key={transaction.id} to={`/dashboard/scheduled/${transaction.id}` as any}>
                        <div className="cursor-pointer rounded-lg border p-3 transition-colors hover:bg-gray-50">
                  ...
                  <div className="py-6 text-center text-gray-500">この日の予定はありません</div>
                )
              ) : (
