...
import { createCurrencyAmount, sumCurrencyAmounts } from '../utils/currency'
...
export const totalAssetsAtom = atom((get) => {
  ...
  const amounts = accounts.map((account) => createCurrencyAmount(account.balance, account.currency))
...
export const assetsByTypeAtom = atom((get) => {
  ...
  const result: Partial<Record<AccountType, ReturnType<typeof createCurrencyAmount>>> = {}
...
export const fixedDepositsAtom = atom((get) => {
  ...
  return accounts.filter(
    (account) => account.type === 'FIXED_DEPOSIT' && account.fixedDepositMaturity
  )
})
...
export const upcomingMaturityDepositsAtom = atom((get) => {
  ...
  return fixedDeposits.filter((account) => account.fixedDepositMaturity! <= thirtyDaysFromNow)
})
...
export const lowBalanceAccountsAtom = atom((get) => {
  ...
  return accounts.filter(
    (account) => account.type !== 'FIXED_DEPOSIT' && Number(account.balance) < threshold
  )
})
...
// 口座削除アクション
export const removeAccountAtom = atom(null, (get, set, accountId: string) => {
  const currentAccounts = get(accountsAtom)
  set(
    accountsAtom,
...
export const updateAccountBalanceAtom = atom(
  null,
  (get, set, accountId: string, newBalance: number | string) => {
    const currentAccounts = get(accountsAtom)
    set(
      accountsAtom,
      currentAccounts.map((account) =>
        account.id === accountId
          ? {
              ...account,
              balance: new Decimal(newBalance) as AppAccount['balance'],
              updatedAt: new Date(),
            }
          : account
      )
...
// カード削除アクション
export const removeCardAtom = atom(null, (get, set, cardId: string) => {
  const currentCards = get(cardsAtom)
  set(
    cardsAtom,
