...
export const upcomingTransactionsAtom = atom((get) => {
  ...
  return transactions
    .filter(
      (tx) =>
        tx.status === 'PENDING' && new Date(tx.dueDate) >= now && new Date(tx.dueDate) <= next30Days
    )
...
export const overdueTransactionsAtom = atom((get) => {
  ...
  return transactions
    .filter((tx) => tx.status === 'PENDING' && new Date(tx.dueDate) < now)
    .sort((a, b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime())
})
...
// Actions（書き込み用atoms）
export const refreshScheduledTransactionsAtom = atom(null, async (get, set) => {
