import { ExchangeRateManager } from '../utils/currency'
...
// 通貨更新アクション
export const updateCurrencyAtom = atom(null, (get, set, updatedCurrency: Currency) => {
  const currentCurrencies = get(currenciesAtom)
  set(
    currenciesAtom,
...
// 基準通貨変更アクション
export const changeBaseCurrencyAtom = atom(null, (get, set, newBaseCurrency: string) => {
  const currencyMap = get(currencyMapAtom)
...
// 最近使用した通貨を追加
export const addRecentCurrencyAtom = atom(null, (get, set, currencyCode: string) => {
...
// 為替レートの最終更新時刻
export const exchangeRateLastUpdateAtom = atomWithStorage<number | null>(
  'kibo-exchangeRateLastUpdate',
  null
)
...
// 為替レート更新時刻を設定
export const setExchangeRateUpdateTimeAtom = atom(null, (get, set) => {
