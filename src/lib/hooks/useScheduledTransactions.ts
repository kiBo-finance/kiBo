...
} from '../atoms/scheduled'

/**
 * Input data for creating a scheduled transaction
 */
...
export function useScheduledTransactions() {
  ...
  const createTransaction = useCallback(
    async (data: CreateScheduledTransactionInput) => {
      setLoading(true)
      try {
  ...
  const updateTransaction = useCallback(
    async (id: string, data: Partial<ScheduledTransaction>) => {
      setLoading(true)
      try {
