...
export async function GET(request: Request) {
  try {
    ...
    return Response.json({ error: '通知設定の取得に失敗しました' }, { status: 500 })
  }
}
  ...
  try {
    ...
    if (validatedData.type === 'WEBHOOK') {
      ...
      try {
        ...
        return Response.json({ error: '有効なWebhook URLを入力してください' }, { status: 400 })
      }
    }
    ...
    return Response.json({ error: '通知設定の保存に失敗しました' }, { status: 500 })
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!settingsId) {
      return Response.json({ error: '設定IDが必要です' }, { status: 400 })
    }
    ...
    if (!settings) {
      return Response.json({ error: '指定された通知設定が見つかりません' }, { status: 404 })
    }
    ...
    return Response.json({ error: '通知設定の削除に失敗しました' }, { status: 500 })
  }
}
