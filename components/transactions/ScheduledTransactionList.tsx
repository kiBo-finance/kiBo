...
export function ScheduledTransactionList({ 
  ...
  if (compact) {
    // コンパクトビュー（ダッシュボード用）
    const displayTransactions = filterStatus === 'OVERDUE' 
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <p className={cn("font-semibold text-sm", getTransactionColor(transaction.type))}>
                  ...
                  {formatAmount(transaction.amount, transaction.currency, transaction.currencyRef?.symbol)}
                </p>
                      ...
                      <TableCell>
                        <TooltipProvider>
                          <Tooltip>
                            ...
                            <TooltipContent>
                              {isPast(new Date(transaction.dueDate)) && transaction.status === 'PENDING' && '期限切れ'}
                              {transaction.status === 'PENDING' && !isPast(new Date(transaction.dueDate)) && '予定'}
                      ...
                      <TableCell>
                        {transaction.isRecurring ? (
                          <div className="flex items-center gap-1">
                            ...
                            <span className="text-sm">{getFrequencyLabel(transaction.frequency)}</span>
                          </div>
                        ) : (
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {selectedTransaction?.isRecurring && '繰り返し設定により、次の予定も自動的に作成されます。'}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>キャンセル</AlertDialogCancel>
            <AlertDialogAction onClick={confirmExecute}>
              実行
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              この操作は取り消すことができません。予定取引「{selectedTransaction?.description}」を完全に削除します。
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>キャンセル</AlertDialogCancel>
