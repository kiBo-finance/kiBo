...
const accountFormSchema = z.object({
  ...
  balance: z.string().regex(/^\d*\.?\d*$/, '正しい金額を入力してください').optional(),
  description: z.string().max(500, '説明は500文字以内で入力してください').optional(),
  fixedDepositRate: z.string().regex(/^\d*\.?\d*$/, '正しい利率を入力してください').optional(),
  ...
  const onSubmit = async (data: AccountFormData) => {
    ...
    try {
      const payload = {
        ...
        fixedDepositRate: isFixedDeposit && data.fixedDepositRate 
          ? parseFloat(data.fixedDepositRate) 
          : undefined,
        fixedDepositMaturity: isFixedDeposit && data.fixedDepositMaturity 
          ? data.fixedDepositMaturity 
          : undefined
      }
      ...
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            {/* 定期預金専用フィールド */}
            {isFixedDeposit && (
              <div className="space-y-4 p-4 border rounded-lg bg-muted/50 dark:bg-gray-800/50 dark:border-gray-600">
                <h3 className="font-medium text-sm">定期預金設定</h3>
