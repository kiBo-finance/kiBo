...
export function ExchangeRatesList({
  ...
  const displayRates = showPopularOnly
    ? exchangeRates.filter(rate => {
        ...
        return popularPairs.includes(pair) || rate.fromCurrency === baseCurrency || rate.toCurrency === baseCurrency
      })
    : exchangeRates
  
  // 通貨ペア別に最新レートのみ取得
  const latestRates = displayRates.reduce((acc, rate) => {
        ...
        {latestRatesList.length === 0 ? (
          ...
          <div className="space-y-3">
            {latestRatesList.map((rate) => {
              const display = getRateDisplay(rate)
              const isBaseCurrencyPair = rate.fromCurrency === baseCurrency || rate.toCurrency === baseCurrency
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isBaseCurrencyPair && "border-primary/20 bg-primary/5 dark:border-primary/30 dark:bg-primary/10"
                  )}
                >
