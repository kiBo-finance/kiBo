...
export async function GET(request: NextRequest) {
  try {
    ...
    const assetsByCurrency = totalsByCurrency.map(item => ({
      ...
      count: item._count.id,
      totalAmount: item._sum.balance || new Decimal(0),
      totalAmountInBase: item._sum.balance ? convertToBase(item._sum.balance, item.currency) : new Decimal(0)
    }))
    
    // まもなく満期の定期預金
    const now = new Date()
    const thirtyDaysLater = new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000)
    const upcomingMaturity = fixedDeposits.filter(account => 
      account.fixedDepositMaturity && account.fixedDepositMaturity <= thirtyDaysLater
    )
    ...
    const stats = {
      ...
      fixedDeposits: {
        ...
        totalAmount: fixedDeposits.reduce((sum, fd) => sum.plus(fd.balance), new Decimal(0)).toNumber(),
