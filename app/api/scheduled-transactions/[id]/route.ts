...
export async function PUT(
  ...
  try {
    ...
    if (validatedData.isRecurring === true && !validatedData.frequency && !existingTransaction.frequency) {
      return NextResponse.json(
        { error: '繰り返し取引には頻度の設定が必要です' },
        { status: 400 }
      )
    }
    ...
    const updatedTransaction = await prisma.scheduledTransaction.update({
      ...
      data: {
        ...
        ...(validatedData.reminderDays !== undefined && { reminderDays: validatedData.reminderDays }),
...
export async function POST(
  ...
  try {
    ...
    const result = await prisma.$transaction(async (prisma) => {
      ...
      if (scheduledTransaction.isRecurring && validatedData.createRecurring && scheduledTransaction.frequency) {
        const nextDueDate = calculateNextDueDate(scheduledTransaction.dueDate, scheduledTransaction.frequency)
