  ...
  try {
    ...
    if (validatedParams.startDate || validatedParams.endDate) {
      ...
      if (validatedParams.startDate) {
        where.dueDate.gte = new Date(validatedParams.startDate)
      }
      if (validatedParams.endDate) {
        where.dueDate.lte = new Date(validatedParams.endDate)
      }
    }
    ...
    const now = new Date()
    prisma.scheduledTransaction.updateMany({
      where: {
        userId: (session.user as any).id,
        status: 'PENDING',
        dueDate: {
