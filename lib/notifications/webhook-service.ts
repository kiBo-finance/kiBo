...
export class WebhookNotificationService {
        ...
        attachments: [
          {
            ...
            footer: 'kiBoアプリ',
            ts: payload.timestamp ? Math.floor(new Date(payload.timestamp).getTime() / 1000) : Math.floor(Date.now() / 1000)
          }
        ]
      }
  ...
  static async processNotification(logId: string) {
    ...
    if (!log || !log.user.notificationSettings.length) {
      await this.updateNotificationLog(logId, 'FAILED', 'No active webhook settings found')
      return
    }

    const settings = log.user.notificationSettings[0]
