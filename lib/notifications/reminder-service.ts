...
export class ReminderService {
   ...
   * 予定取引リマインダーメッセージを生成
   */
  static generateReminderMessage(transaction: any): NotificationPayload {
    const amount = new Intl.NumberFormat('ja-JP', {
      style: 'currency',
      currency: transaction.currency,
      minimumFractionDigits: transaction.currencyRef.decimals,
      maximumFractionDigits: transaction.currencyRef.decimals,
    ...
    const daysUntilDue = Math.ceil((new Date(transaction.dueDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24))
   ...
   * 期限切れ通知メッセージを生成
   */
  static generateOverdueMessage(transaction: any): NotificationPayload {
    const amount = new Intl.NumberFormat('ja-JP', {
      style: 'currency',
      currency: transaction.currency,
      minimumFractionDigits: transaction.currencyRef.decimals,
    ...
    for (const transaction of reminders) {
      ...
      try {
        ...
        if (result.success) {
          ...
          console.error(`❌ Failed to send reminder for transaction: ${transaction.description}`, result.error)
        }
      } catch (error) {
        console.error(`❌ Error processing reminder for transaction: ${transaction.description}`, error)
      }
    }
    ...
    for (const transaction of overdueTransactions) {
      ...
      try {
        ...
        if (result.success) {
          ...
          // 送信失敗
          await WebhookNotificationService.updateNotificationLog(log.id, 'FAILED', result.error)
          console.error(`❌ Failed to send overdue notification for transaction: ${transaction.description}`, result.error)
        }
      } catch (error) {
        console.error(`❌ Error processing overdue notification for transaction: ${transaction.description}`, error)
      }
    }
